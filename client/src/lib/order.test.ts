...
const input = { orderId: "SB-20260827-A7K3", lines: resolveCartItems(items), totals: calculateOrderTotals(items, "priority"), details: { name: "Arun", topic: "Consumer Behaviour", deadline: "2026-08-29T18:00", delivery: "priority" as const, notes: "Please use the department format." } };

describe("order communication", () => {
  it("makes the expected local conversation reference", () => expect(createOrderReference(new Date(2026, 7, 27), () => 0.01)).toBe("SB-20260827-AAAA"));
  ...
  it("creates URL-encoded official-style WhatsApp and Telegram URLs", () => {
    ...
    expect(createTelegramOrderUrl("softbazzar_orders", message)).toBe(`https://t.me/softbazzar_orders?text=${encodeURIComponent(message)}`);
  });
