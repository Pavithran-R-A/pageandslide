...
export function ServiceCatalogue({ onAdded }: ServiceCatalogueProps) {
  ...
  return <section className="catalogue-section" id="services" aria-labelledby="services-title">
    <div className="catalogue-heading"><p className="eyebrow">SERVICE CATALOGUE</p><h2 id="services-title">Made to read clearly.<br />Priced the same way.</h2><p>Choose the format and scale that reflects your requirement. Every applicable service is delivered in an editable format.</p></div>
    <div className="catalogue-list">{SERVICE_CATEGORIES.map((service) => <article className={`service-entry${service.featured ? " service-entry--featured" : ""}`} key={service.id}>
      <div className="service-intro"><p className="service-index">{service.number}</p><h3>{service.name}</h3><p>{service.description}</p></div>
      <div className="service-tiers" aria-label={`${service.name} pricing`}>{service.tiers.map((tier) => {
